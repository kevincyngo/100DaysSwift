...
//possible ways to create enemies
//CaseIterable protocol puts the enum in an array
//This lets us to randomElement() to get a random case from the enum
    ...
    override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        for case let node as SKSpriteNode in nodesAtPoint {
            if node.name == "enemy" {
                ...
                // Make pengiun scale out and fade at same time
                let scaleOut = SKAction.scale(to: 0.001, duration:0.2)
                let fadeOut = SKAction.fadeOut(withDuration: 0.2)
                
                //SKAction.group runs it at same time
                let group = SKAction.group([scaleOut, fadeOut])
                
                // After making the penguin scale out and fade out, we should remove it from the scene.
                //SKAction.sequence runs it in sequence
